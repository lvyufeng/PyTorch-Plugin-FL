...
The device-side capability rows are a separate question from the memcpy/memset pair, and Ascend's were the last held back. `profiler_support.capabilities_for_platform` set `device = platform != "ascend"` and propagated that to `kernel`, `runtime`, `flow`, `linkage`, and `metadata`, so ten of the contract's twelve cases skipped on Ascend and the two that ran could not fail. That row described the tracer as it stood before MSPTI activity capture, not the tracer as it is. Measured on 910 with CANN 9.0 and the preload above, ten of the twelve pass, one skips (memset), and the twelfth xfails on linkage. Re-run without the preload the same way and the result is `9 passed, 2 skipped` — the preload gates memcpy alone; kernel and runtime/API records arrive through `CannDeviceTracer`'s lazy `dlopen`, which does not need it.

Opening those rows does not make device-time linkage work. Ascend links each device event to the CPU op that *follows* its launch rather than to the op that launched it: the five `MatMulV2_*` kernels dispatched by `aten::matmul` carry the `External id`s of the `aten::empty` calls that follow, the five `relu_forward_kernel_rank_1_0` kernels dispatched by `aten::relu` carry those of the `aten::empty_strided` calls, and 24 of the 37 CPU-op ids in the shared workload own no device event at all. The duration is not lost — `aten::matmul.device_time_total` reads 98.0us against a `self_device_time_total` of 0.0us, so it arrives through Kineto's child aggregation — but the launching operator itself reads zero, which is what `test_profiler_device_time_linkage` selects on. The `linkage` row is deliberately left True so that case reports as an XFAIL rather than disappearing into a skip; the tracer-side cause is issue #425.

That case carried an unconditional `xfail` until this change, added by #272 when `mm`/`bmm` moved from `cuda` to `flaggems` and the CUDA matmul stopped surfacing device time (FlagGems issue #6223). Unconditional was the wrong shape. `xfail` is not a skip — it runs the body and absorbs the outcome — so an all-platform marker does not narrow the contract, it converts the assertion's failures into XFAILs on every platform, including the four the known defect is not about. DCU, PPU and MUSA had each been recording `1 xpassed` in their profiler-contract runs, which is the assertion passing under a marker that would equally have swallowed it failing. The marker is now conditioned on the two platforms with a measured reason to need it, `cuda` and `ascend`, and the reason names both; the rest assert.
